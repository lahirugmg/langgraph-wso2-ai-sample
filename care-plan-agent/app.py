...
def _call_llm(messages: List[dict[str, str]]) -> Optional[str]:
    ...
    logger.info("=" * 80)
    logger.info("🤖 LLM API CALL - Care Plan Generation")
    logger.info("=" * 80)
    ...
    for attempt in range(1, LLM_MAX_ATTEMPTS + 1):
        try:
            ...
            logger.info("✓ LLM plan generation completed successfully")
            logger.info("=" * 80)
            logger.info("")
            return content
            ...
            if attempt == LLM_MAX_ATTEMPTS:
                raise RuntimeError("LLM call timed out while awaiting upstream response") from exc
            delay = backoff
            if delay:
            ...
            if status == 504:
                logger.error("✗ Propagating upstream timeout (504) to caller for retry handling")
                raise RuntimeError("LLM call failed due to upstream timeout (504)") from exc
            ...
            logger.exception("✗ LLM call failed while parsing response: %s", exc)
            return None
